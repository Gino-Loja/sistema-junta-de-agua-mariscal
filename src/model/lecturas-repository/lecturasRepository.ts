...
export interface ILecturesRepository {
    ...
    getComsumedMetersByMonths: (month: number, year: number) => Promise<QueryResultError<{ exceso: number , consumo: number  }>>;
    ...
    createLecture: (formData: LecturesDto) => Promise<QueryResultError<boolean>>;
    updateLecture: (formData: LecturesDto, id: number) => Promise<QueryResultError<boolean>>;
    ...
    getMeasurementMacroAreaChart: ( from: string, to: string,  month: number, year: number) => Promise<QueryResultError<{ fecha: string, consumo: number }[]>>;
