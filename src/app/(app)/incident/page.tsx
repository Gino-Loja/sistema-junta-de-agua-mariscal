...
export default async function Page({ searchParams }: PageProps) {
  const { date, query, page, per_page, sector, month, year } =
    ...
    <div className="flex min-h-screen w-full flex-col bg-muted/40 mt-4">
      ...
      <div className="flex flex-col sm:gap-4 pb-4">
        <div className="flex justify-between gap-4 px-6">
          <h1 className="text-2xl font-bold shrink  max-w-42 border-divider rounded-xl">
...
async function CardGetTotalAmountCostIncidetByYear({
  repository,
  year,
  month,
}: {
  repository: IServiceIncidentRepository;
  year: number;
  month: number | null;
}) {
  const getTotalAmountCostIncidetByYear =
    await repository.getTotalAmountCostIncidetByYear(year, month);
  if (!getTotalAmountCostIncidetByYear.success) {
    return (
      <Alert
        color={"danger"}
        title={`!${getTotalAmountCostIncidetByYear.error}`}
      />
    );
  }
      ...
      <div>
        <div className="space-y-3">
          <h4 className={`text-3xl font-bold`}>
            {"$" + getTotalAmountCostIncidetByYear.data.toFixed(2)}
          </h4>
          <p className="text-sm text-gray-500 ">
            {" "}
            Resumen del total de Gastos en el Año
          </p>
        </div>
      </div>
    </Card>
...
async function CardGetTotalIncidentByYear({
  repository,
  year,
  month,
}: {
  ...
  const getTotalIncidentByYear = await repository.getTotalIncidentByYear(
    year,
    month,
  );
  if (!getTotalIncidentByYear.success) {
    return (
      <Alert color={"danger"} title={`!${getTotalIncidentByYear.error}`} />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div>
        <div className="space-y-3">
          <h4 className={`text-3xl font-bold`}>
            {getTotalIncidentByYear.data}
          </h4>
          <p className="text-sm text-gray-500 ">
            {" "}
            Resumen del total de Incidentes en el Año
          </p>
        </div>
      </div>
    </Card>
...
async function CardGetTotalIncidentBySector({
  repository,
  year,
  month,
}: {
  ...
  const getTotalIncidentBySector = await repository.getTotalIncidentBySector(
    year,
    month,
  );
  if (!getTotalIncidentBySector.success) {
    return (
      <Alert color={"danger"} title={`!${getTotalIncidentBySector.error}`} />
    );
  }
  ...
  return <CircleChartCard {...data}></CircleChartCard>;
}

async function FechtRenderPaginationControls({
  repository,
  selectedDate,
  page,
  per_page,
  ...
  const data = await repository.getCounterIncidentPagination(
    selectedDate,
    query,
    sectorId,
    year,
