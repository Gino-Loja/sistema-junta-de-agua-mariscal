...
export default function Page({ searchParams }: PageProps) {
  const { date } = coordinatesCache.parse(searchParams)
      ...
      <div className="container mx-auto p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
          <Suspense fallback={<SkeletonStatsCard />}>
            <GetRenderTotalIncident repository={repository} date={date}></GetRenderTotalIncident>
          </Suspense>
          <Suspense fallback={<SkeletonStatsCard />}>
            < GetRenderTotalInvoice repository={repository} date={date}></GetRenderTotalInvoice>
          </Suspense>
          <Suspense fallback={<SkeletonStatsCard />}>
            <GetRenderTotalSheets repository={repository} date={date}></GetRenderTotalSheets>
          </Suspense>
          <Suspense fallback={<SkeletonStatsCard />}>
            <GetRenderTotalWaterMeter repository={repository} date={date}></GetRenderTotalWaterMeter>
          </Suspense>
          <Suspense fallback={<SkeletonStatsCard />}>
          ...
          <Suspense fallback={<SkeletonStatsCard />}>
            <GetRenderAmountInvoice repository={repository} date={date}></GetRenderAmountInvoice>
          </Suspense>
          <Suspense fallback={<SkeletonStatsCard />}>
            <GetRenderAmountIncident repository={repository} date={date}></GetRenderAmountIncident>
          </Suspense>
          <Suspense fallback={<SkeletonStatsCard />}>
            < GetRenderAmountSheets repository={repository} date={date}></GetRenderAmountSheets>
          </Suspense>
...
async function GetRenderTotalIncident({ repository, date }: { repository: IServiceDashboardRepository, date: string }) {
...
async function GetRenderTotalInvoice({ repository, date }: { repository: IServiceDashboardRepository, date: string }) {
...
async function GetRenderTotalSheets({ repository, date }: { repository: IServiceDashboardRepository, date: string }) {
...
async function GetRenderTotalWaterMeter({ repository, date }: { repository: IServiceDashboardRepository, date: string }) {
...
//amount
async function GetRenderAmountInvoice({ repository, date }: { repository: IServiceDashboardRepository, date: string }) {
...
async function GetRenderAmountIncident({ repository, date }: { repository: IServiceDashboardRepository, date: string }) {
  ...
  return (<StatsCard
    title="Ingresos en Incidentes"
    value={amountIncident.data.toFixed(2)}
    description={`Monto recaudado de incidentes`}
  >
    <div className="p-3 rounded-full bg-success-500 ">
      <CircleDollarSign className="h-6 w-6" />
...
async function GetRenderAmountSheets({ repository, date }: { repository: IServiceDashboardRepository, date: string }) {
