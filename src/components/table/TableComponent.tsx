...
import { User as Users } from "@/model/User";
...
const INITIAL_VISIBLE_COLUMNS = ["nombre", "email", "estado", "actions"];
...
export default function TableComponent() {
  ...
  const columns = [
    ...
    { name: "AGE", uid: "direccion", sortable: true },
    { name: "EMAIL", uid: "email" },
    { name: "STATUS", uid: "estado", sortable: true },
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (statusFilter !== "all" && Array.from(statusFilter).length !== statusOptions.length) {
      ...
      filteredUsers = filteredUsers.filter((user) => {
        let status = user.estado ? "true" : "false";
        return Array.from(statusFilter).includes(status)
        //console.log(Array.from(statusFilter).includes(status))
      }
      );
    }
  ...
  const renderCell = React.useCallback((user: Users, columnKey: React.Key) => {
    ...
    switch (columnKey) {
      ...
      case "estado":
        return (
          <Chip className="capitalize" color={statusColorMap[user.estado ? "true" : "false"]} size="sm" variant="flat">
    ...
    return (
      <div className="py-2 px-2 flex justify-between items-center">
        ...
        <Pagination
          ...
          page={page}
          total={pages}
          disableCursorAnimation	={pages === 0}
          onChange={setPage}
        />
